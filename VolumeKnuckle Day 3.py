...
#
DEAD_ZONE_TOP = 0.15      # Top 15% of frame = Volume 100
DEAD_ZONE_BOTTOM = 0.85   # Bottom 15% of frame = Volume 0
...
def fist_to_volume(y_normalized):
    ...
    # 1. Handle the "Top" Dead Zone (y near 0 should be 100% volume)
    if y_normalized <= DEAD_ZONE_TOP:
        return 100.0
    
    # 2. Handle the "Bottom" Dead Zone (y near 1 should be 0% volume)
    elif y_normalized >= DEAD_ZONE_BOTTOM:
        return 0.0

    # 3. Map the active range linearly
    # Input range: [DEAD_ZONE_TOP, DEAD_ZONE_BOTTOM]
    # Output range: [100.0, 0.0] 
    # (Notice 100 comes first because the top of the screen has a smaller y-value)
    volume = np.interp(
        y_normalized, 
