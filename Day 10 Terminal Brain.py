...
def build_llm_prompt(error_text):
    return f"""
You are a terminal error fixer.
...
Rules:
- 1–2 sentences
- If command → show exact command
- If code → show exact change
- No explanations
...
ERROR_PATTERNS = [
    ...
    r"^\w*Error:", # Matches NameError:, TypeError:, etc. at start of line
    r"^\w*Exception:",
    ...
    r"Permission denied",
    r"Exec format error", # Common in cross-platform (M1 vs Intel)
    
    # Generic status signals
    r"EXIT STATUS [1-9]",
    r"FAILED",
    r"CRITICAL",
    r"Segmentation fault", # The ultimate "bad news"
]
...
def get_fix(error_text):
    # If we already fixed this error before, reuse it
    if error_text in fix_cache:
        return fix_cache[error_text]
